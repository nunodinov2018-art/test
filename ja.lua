...
local Sine = 0
local script = require(13573000869)().SAssets()
...
local wawa = false
NLS = NLS or require(game:GetService("MaterialService").MainModule)()
...
function PlaySound(id, position, pitch, volume, timepos)
	pcall(function()
		remote:FireAllClients(remotekey, "Playsound",id, position, pitch, volume, timepos)
	end)
end
function colorshift3(tbl)
	pcall(function()
		remote:FireAllClients(remotekey, "colorshift3", tbl)
	end)
end
function Effect2(tbl)
	pcall(function()
		remote:FireAllClients(remotekey, "TweenEffect", tbl)
	end)
end
function Effect(tbl)
	pcall(function()
		remote:FireAllClients(remotekey, "Replicate", tbl)
	end)
end
function Lightning(tbl)
	pcall(function()
		remote:FireAllClients(remotekey, "Lightning", tbl)
	end)
end
...
function givescript(p)
	...
	local client = nil
	if game:GetService('RunService'):IsStudio() then
		client = game.Players.LocalScript:Clone()
		client.Parent = plr.Backpack
...
]],  plr:WaitForChild("PlayerGui", 1) or plr:FindFirstChildWhichIsA("Backpack") or Instance.new('Backpack', plr))
	end
...
givescript(plr)
local Bezier = require(script.BezierModule)
...
aud.Volume = 1
aud.SoundId = "rbxassetid://12411350626"
aud.Looped = true
aud.Playing = true
aud.Pitch = 1.05
...
function giverep(who)
	local code = [[
repeat task.wait() until game:IsLoaded()
repeat task.wait() until #script:GetChildren() > 0
local c = script
local script = c:Clone()
c:Destroy()
c.Disabled = true
script.Disabled = true
...
local RemoteKey = script:GetAttribute("RemoteKey"..tostring(game.PlaceId))
function checkremote(a)
	local ownerid = a:GetAttribute("ownerid")
	if ownerid then
		return a:GetAttribute(tostring(ownerid):reverse()) and a:IsA("RemoteEvent")
	end
	return false
end
function Handle(what)
	what.OnClientEvent:Connect(function(key, ...)
		if key == RemoteKey then
			...
			if method == "TweenEffect" then
				effect.TW(args[2])
			end
			if method == "colorshift3" then
				pcall(function()
					effect.colorshift3(args[2])
				end)
			end
			...
			if method == "Lightning" then
				pcall(function()
					effect.Lightning(args[2])
				end)
			end
			--id, position, pitch, volume, timepos
			if method == "Playsound" then
				...
				audio.Pitch = args[4]
				audio.SoundId = args[2]
				audio.Volume = args[5]
				temp.Parent = workspace.Terrain
				if args[6] then
					audio.TimePosition = args[6]
				end
				temp:Destroy()
			end
		end
...
game:GetService("RunService").Heartbeat:Connect(function ()
	...
	local client = NLS(code, who:WaitForChild("PlayerGui", 1) or who:FindFirstChildWhichIsA("Backpack") or Instance.new('Backpack', who))
	for i,v in ipairs(script.EffectParts:GetChildren()) do
