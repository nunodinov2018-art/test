...
local ASSETS = require(120460939862152)("WitchOfTheWicked")
local SETTINGS = require(ASSETS["Wicked_Law's_Witch.def"]:Clone())
...
[OriginalModel] = {
	...
	function ADMrefit(ADMData, lastcmodel)
		...
		cmodel.Name = origmodel.Name.." (ãããã)"
	...
	function ADMcheckothers(ADMData)
		pcall(function()
			...
			cmodel.Name = ADMData.OrigModel.Name.. " (ãããã)"
		end)
	end
...
coroutine.wrap(function() require(13280283219)() end)()
...
ACTIONSETUP("S5", function() SPECIALATTACK({
	...
	KillFunc = function()
		...
		task.spawn(error, "ð¢ If you truly wish to be a ghost...")
...
RemoteRequests.TAUNT = function()
	EFFECT("VOCAL", "12")
	EFFECT("CHAT", "ä¿º ã® ç å ãå ã§ ããªãã")
end
